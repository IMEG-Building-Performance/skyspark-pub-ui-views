...
window.EventCostV2.annotations = {};

/**
 * Draw the annotation overlay with all events
 * Handles rendering order so hovered events appear on top
 */
window.EventCostV2.annotations.drawAnnotationOverlay = function(chartInstance, overlayCanvas, events) {
  ...
  var labelPositions = window.EventCostV2.labels.calculateLabelPositions(
    events,
    xScale,
    rawChartArea
  );

  var hoverState = window.EventCostV2.state.hoverState;
  var visibilityState = window.EventCostV2.state.visibilityState;
  ...
  events.forEach(function(event, index) {
    ...
    if (isSpanEvent) {
      ...
      window.EventCostV2.annotations.drawSpanAnnotation(
        ctx,
        startPixel,
        endPixel,
        chartArea,
      ...
      if (rawXPixel >= rawChartArea.left && rawXPixel <= rawChartArea.right) {
        ...
        window.EventCostV2.annotations.drawAnnotationLine(
          ctx,
          xPixel,
          chartArea,
          event.color,
        ...
        window.EventCostV2.labels.drawAnnotationLabel(
          ctx,
          xPixel,
          labelPos,
          event.label,
  ...
  if (highlightedIndex >= 0 && visibilityState[highlightedIndex] !== false) {
    ...
    if (isSpanEvent) {
      ...
      if (rawXPixel >= rawChartArea.left && rawXPixel <= rawChartArea.right) {
        ...
        window.EventCostV2.tooltips.drawHoverMarker(
          ctx,
          chartInstance,
          xPixel,
          highlightedEvent
  ...
  if (hoverState.hoveredEvent) {
    window.EventCostV2.tooltips.drawRichTooltip(
      ctx,
 ...
 */
window.EventCostV2.annotations.drawAnnotationLine = function(ctx, xPixel, chartArea, color, isHovered) {
  var style = window.EventCostV2.state.annotationStyle;
  var s = (window.EventCostV2.state.responsiveScaling || {}).vhScale || 1.0;
 ...
 */
window.EventCostV2.annotations.drawSpanAnnotation = function(ctx, startX, endX, chartArea, color, isHovered, anyHovered) {
  var style = window.EventCostV2.state.annotationStyle;
  var s = (window.EventCostV2.state.responsiveScaling || {}).vhScale || 1.0;
