/**
 * loadPowerData.js — Time-series power data for Site Status chart (Event Cost V2)
 */

window.EventCostV2 = window.EventCostV2 || {};
window.EventCostV2.api = window.EventCostV2.api || {};

window.EventCostV2.api.loadPowerData = function(siteRef, startDate, endDate, utilityType) {
  ...
  return evalAxon(axonExpr).then(function(data) {
    ...
    return data.rows.map(function(row) {
      ...
      if (val === null) {
        for (var key in row) {
          if (key !== 'Timestamp' && key !== 'ts') { val = extractValue(row[key]); break; }
        }
      }
