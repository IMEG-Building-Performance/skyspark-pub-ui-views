...
window.EventCostV2.generators = {};

/**
 * Generate demo eventCostResult records.
 * Used as a fallback when no real SkySpark records are found.
 * Dates are relative to today so the demo stays current.
 */
window.EventCostV2.generators.generateDemoEventCostResults = function() {
  var today = new Date();
  ...
  // Past events have costs; future events have no costs (show as Pending)
  var defs = [
  ...
  defs.forEach(function(def) {
    ...
    if (utilities.length === 0) {
      // Future event — one placeholder row so it appears in aggregateEventSummaries
      results.push({
        eventID: def.id, eventName: def.name, siteRef: 'demo',
        utilityType: 'Electric', costType: 'Usage',
        usage: 0, rate: 0, cost: 0, eventSF: def.sf,
        metersUsed: 0, calculatedOn: null, dataQuality: null,
        errorCount: 0, mostCommonError: null,
      ...
      utilities.forEach(function(u) {
        ...
        results.push({
          eventID: def.id, eventName: def.name, siteRef: 'demo',
          utilityType: u, costType: 'Usage',
          usage: rate > 0 ? cost / rate : 0,
          rate: rate, cost: cost, eventSF: def.sf,
          metersUsed: utilities.length, calculatedOn: endDate,
          dataQuality: def.dq, errorCount: 0, mostCommonError: null,
          eventStart: startDate, eventEnd: endDate
        });
      });
    }
 ...
 */
window.EventCostV2.generators.generateSampleData = function(startDateStr, endDateStr) {
  var data = [];
 ...
 */
window.EventCostV2.generators.generateSampleEvents = function() {
  return [
