/**
 * EventDetailV2.js — Tab 3: Single-event cost deep dive
 *
 * Renders:
 *  - Header card (event name, total cost, metadata)
 *  - Per-site utility breakdown (from eventCostResult records)
 *  - Spaces & Meters section (REAL DATA from eventCost_bookings)
 *  - Concurrent events section (REAL DATA from report_EventsByDate)
 *
 * @param {HTMLElement} container
 * @param {Object} eventSummary  — aggregated event object from MonthlyOverview
 * @param {Array}  allResults    — full eventCostResults array (for per-utility detail)
 * @param {Function} onBack      — called when user clicks Back
 * @param {Function} onSelectEvent — called when user clicks a concurrent event row
 */

window.EventCostV2 = window.EventCostV2 || {};
window.EventCostV2.eventDetailV2 = {};

window.EventCostV2.eventDetailV2.render = function(container, eventSummary, allResults, onBack, onSelectEvent) {
