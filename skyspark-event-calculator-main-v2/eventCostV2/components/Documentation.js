/**
 * Documentation.js — Tab 5: Methodology and user guide
 */

window.EventCostV2 = window.EventCostV2 || {};
window.EventCostV2.documentation = {};

window.EventCostV2.documentation.renderTab = function(container) {
  container.innerHTML = '';
  container.style.padding = '24px 28px';

  // Role-based visibility: TODO — add role check here.
  // If the SkySpark user has role "client", show only the User Guide section.
  // If the user has role "admin" or "engineer", show the full Methodology section too.
  // Check role via: view.session().user().roles().contains("admin")
  ...
  // ── Methodology (engineers / admins only) ───────────────────────
  if (showMethodology) {
