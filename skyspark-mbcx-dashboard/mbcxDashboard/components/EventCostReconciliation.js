...
window.mbcxDashboard.eventCost.utilityReconciliation = {};

window.mbcxDashboard.eventCost.utilityReconciliation.render = function(container, eventCostResults) {
  ...
  var currentUtility = window.mbcxDashboard.eventCost.state.reconciliationUtility || 'Electric';
  ...
  var utilityBar = interactions.createCostUtilityToggle(currentUtility, function(u) {
    currentUtility = u;
    window.mbcxDashboard.eventCost.state.reconciliationUtility = u;
    renderContent();
  });
  ...
  function buildChart(months, eventCostByMonth, billCostByMonth, utilColor) {
    ...
    loader.loadChartJs(function() {
      ...
      window._ecReconcChart = new Chart(chartCanvas.getContext('2d'), {
        type: 'bar',
        data: {
          labels: labels,
          datasets: [
