...
window.mbcxDashboard.eventCost.expandView = {};

window.mbcxDashboard.eventCost.expandView.open = function(state, interactions, annotations, timeline, chart) {
  ...
  var expS = (window.mbcxDashboard.eventCost.state.responsiveScaling || {}).vhScale || 1.0;
  ...
  if (interactions.createEventList) {
    interactions.createEventList(sidebar, currentEvents, null);
  } else {
    sidebar.innerHTML = '<div style="padding:20px;color:#6c757d;font-size:13px;text-align:center;">Event list not available.</div>';
  }
