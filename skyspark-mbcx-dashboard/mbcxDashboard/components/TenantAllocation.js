// components/TenantAllocation.js
// Tenant Usage Allocation tab — ported from skyspark-tenant-meter-allocation.
// Renders inside the MBCx Dashboard content area.
window.mbcxDashboard = window.mbcxDashboard || {};
window.mbcxDashboard.components = window.mbcxDashboard.components || {};

(function (NS) {

  var EVALS = window.mbcxDashboard.evals;
  ...
  function fmtNum(v) {
    ...
    var neg = v < 0; var abs = Math.abs(v); var s;
  ...
  function fmtPct(v) { return (v == null || isNaN(v)) ? '0.00%' : v.toFixed(2) + '%'; }
  function fmtBtu(val, unit) {
    if (val == null || isNaN(val)) return '—';
    return (!unit || unit === 'BTU') ? fmtNum(val / 1000) : fmtNum(val);
  }
  function btuUnit(unit) { return (!unit || unit === 'BTU') ? 'kBTU' : unit; }
  function _esc(s) {
  ...
  // ── Data helpers ─────────────────────────────────────────────────────────────
  function _getRows() { return (_state.allData && _state.allData[_state.selectedUtil]) || []; }

  function _groupRows(rows) {
    var map = {}, order = [];
    rows.forEach(function (row) {
      if (!map[row.groupId]) {
        map[row.groupId] = { id: row.groupId, name: row.groupName, meters: [], totalUsage: 0, totalPercOfPlant: 0, totalCost: 0 };
  ...
  function _pctBar(pct, color) {
    ...
    return '<div class="ma-pct-wrap">' +
      '<div class="ma-pct-track"><div class="ma-pct-fill" style="width:' + w + '%;background:' + color + '"></div></div>' +
      '<span class="ma-pct-label" style="color:' + color + '">' + fmtPct(pct) + '</span></div>';
  }
  ...
  function _renderTable(rows) {
    ...
    var rowsHtml = sorted.map(function (m, idx) {
      ...
      return '<div class="ma-meter-row' + (idx % 2 === 0 ? '' : ' stripe') + '">' +
        '<span class="ma-meter-name" title="' + _esc(m.meterName) + '">' + _esc(mName) + '</span>' +
        '<span class="ma-meter-usage" style="color:' + cfg.color + '">' +
          fmtBtu(m.usage, m.usageUnit) + '<small>&nbsp;' + _esc(btuUnit(m.usageUnit)) + '</small></span>' +
        '<span class="ma-meter-pct">' + _pctBar(m.percOfPlant, cfg.color) + '</span></div>';
    }).join('');
    return '<div class="ma-card ma-table-card">' +
      '<div class="ma-table-titlebar">' +
        '<span class="ma-table-title">Meter Allocation Detail</span>' +
        '<span class="ma-table-hint">All meters sorted by % of plant</span></div>' +
      '<div class="ma-col-headers">' +
        ...
        '<span class="ma-col-h sortable" data-sort="pct">% of Plant ' + sortArrow('pct') + '</span></div>' +
      '<div class="ma-rows">' + rowsHtml + '</div></div>';
  }
  ...
  function _renderSummaryPage() {
    ...
    var kpiHtml = '<div class="ma-kpi-strip">' +
      kpiCards.map(function (k) {
        return '<div class="ma-kpi-card"><div class="ma-kpi-label">' + k.label + '</div>' +
          '<div class="ma-kpi-value" style="color:' + k.color + '">' + k.value + '</div></div>';
      }).join('') + '</div>';
    ...
    if (pendingUtils.length > 0) {
      ...
      noticeHtml = '<div class="ma-sum-notice">' +
        ...
        '<code>report_meterValidation_plantTotalsTable</code> not yet available for those utilities.</span></div>';
    }
    ...
    UTIL_KEYS.forEach(function (u) {
      ...
      detailRows += '<tr class="ma-recon-row">' +
        ...
        '<td class="ma-recon-num">' + (tenantCost != null ? fmtCost(tenantCost) : '—') + '</td></tr>';
    });
    ...
    var detailCard = selected
      ? '<div class="ma-card ma-table-card">' +
          '<div class="ma-table-titlebar">' +
            ...
            '<span class="ma-table-hint">Cost = (tenant kBTU &divide; plant kBTU) &times; plant cost</span></div>' +
          '<table class="ma-recon-table"><thead><tr>' +
    ...
    var reconcRows = UTIL_KEYS.map(function (u) {
      ...
      var diffColor = pctDiff == null ? '#9ca3af' : Math.abs(pctDiff) <= 5 ? '#16a34a' : Math.abs(pctDiff) <= 20 ? '#d97706' : '#dc2626';
      return '<tr class="ma-recon-row">' +
        ...
        '<td class="ma-recon-num" style="color:#6b7280">' +
          (variance != null ? (variance >= 0 ? '+' : '') + fmtBtu(variance, rawBtuUnit) + '<small>&nbsp;' + dispUnit + '</small>' : '—') + '</td>' +
        '<td class="ma-recon-num">' +
          (pctDiff != null ? '<span style="color:' + diffColor + ';font-weight:700">' + (pctDiff >= 0 ? '+' : '') + pctDiff.toFixed(1) + '%</span>' : '—') +
        '</td></tr>';
    }).join('');
    ...
    var reconcTableHtml = '<div class="ma-card ma-table-card">' +
      '<div class="ma-table-titlebar">' +
        ...
        '<span class="ma-table-hint">Plant total vs. authoritative tenant totals</span></div>' +
      '<table class="ma-recon-table"><thead><tr>' +
        '<th class="ma-recon-head">Utility</th>' +
      ...
      '</tr></thead><tbody>' + reconcRows + '</tbody></table></div>';

    var reconcSectionHtml = _collapseSection('reconciliation', 'BTU Reconciliation', _state.expanded.reconciliation, reconcTableHtml);

    return '<div class="ma-page">' + noticeHtml + kpiHtml + billingSectionHtml + reconcSectionHtml +
      '<div class="ma-footer">Cost&nbsp;=&nbsp;(Tenant&nbsp;kBTU&nbsp;&divide;&nbsp;Plant&nbsp;kBTU)&nbsp;&times;&nbsp;Plant&nbsp;Cost&nbsp;&nbsp;&middot;&nbsp;&nbsp;SkySpark&nbsp;pUb</div></div>';
  }
  ...
  function _renderResidentialPage() {
    ...
    var utilSelector = '<div class="ma-util-selector">' +
      RES_UTILS.map(function (k) {
        var u = UTILS[k]; var isActive = k === activeUtil;
    ...
    if (!data) {
      return '<div class="ma-page">' + utilSelector +
        '<div class="ma-card ma-empty"><div>No residential data loaded for ' + _esc(activeUtil) + '.</div></div></div>';
    }
    ...
    var diagramHtml = '<div class="ma-card">' +
      '<div class="ma-table-titlebar">' +
        ...
        '<span class="ma-table-hint">Total = (G1+G2) + (G3&#8722;G5) + (G4&#8722;G5&#8722;G6)</span></div>' +
    ...
    var calcCard = '<div class="ma-card ma-table-card">' +
      '<div class="ma-table-titlebar"><span class="ma-table-title">Calculation Steps</span>' +
        '<span class="ma-table-hint">Intermediate values for each term</span></div>' +
      '<table class="ma-recon-table"><thead><tr>' +
        '<th class="ma-recon-head">Component</th><th class="ma-recon-head">Expression</th>' +
        '<th class="ma-recon-head right">Value</th></tr></thead>' +
      '<tbody>' + stepRows + totalRow + '</tbody></table></div>';

    function meterTable(meters) {
      if (!meters || !meters.length) return '<div style="padding:12px 20px;color:#9ca3af;font-size:12px">No meters in this group.</div>';
    ...
    var groupMeterSections = [
      ...
      return '<div class="ma-collapse-section">' +
        '<button class="ma-section-toggle" data-collapse="' + g.key + '" style="border-left:3px solid ' + accentColor + '">' +
          ...
          '<span class="ma-toggle-chevron">' + (isOpen ? '&#9660;' : '&#9658;') + '</span></button>' +
        (isOpen ? '<div class="ma-section-body"><div class="ma-card ma-table-card" style="margin:0">' + meterTable(g.meters) + '</div></div>' : '') +
      '</div>';
    }).join('');

    return '<div class="ma-page">' + utilSelector + diagramHtml + calcCard + groupMeterSections +
  ...
  function _renderBody() {
    ...
    if (_state.page === 'summary') { body.innerHTML = _renderSummaryPage(); return; }
    if (_state.page === 'residential') { body.innerHTML = _renderResidentialPage(); return; }
    ...
    var utilSelector = '<div class="ma-util-selector">' +
      UTIL_KEYS.map(function (k) {
        var u = UTILS[k]; var isActive = k === _state.selectedUtil;
        return '<button class="ma-util-pill' + (isActive ? ' is-active' : '') + '" data-util-tab="' + k + '" ' +
          'style="' + (isActive ? 'background:' + u.color + ';color:#fff;border-color:' + u.color + ';' : '') + '">' +
          u.icon + '&nbsp;' + u.label + '</button>';
      }).join('') + '</div>';

    body.innerHTML = '<div class="ma-page">' + utilSelector + _renderKpis(rows) + _renderTable(rows) +
      '<div class="ma-footer">Cost&nbsp;=&nbsp;(Meter&nbsp;Usage&nbsp;÷&nbsp;Plant&nbsp;Output)&nbsp;&times;&nbsp;Rate&nbsp;&nbsp;&middot;&nbsp;&nbsp;SkySpark&nbsp;pUb</div></div>';
  }
  ...
  function _attachListeners() {
    ...
    _container.addEventListener('click', function (e) {
      ...
      if (pageTab) {
        ...
        if (pg && pg !== _state.page) { _state.page = pg; _renderAll(); }
        return;
      }
      var collapseBtn = e.target.closest('[data-collapse]');
      if (collapseBtn) {
        var section = collapseBtn.getAttribute('data-collapse');
        if (section && _state.expanded) { _state.expanded[section] = !_state.expanded[section]; _renderBody(); }
        return;
      }
      var tenantTab = e.target.closest('[data-tenant-tab]');
      if (tenantTab) {
        var tenant = tenantTab.getAttribute('data-tenant-tab');
        if (tenant && tenant !== _state.selectedTenant) { _state.selectedTenant = tenant; _renderBody(); }
        return;
      }
      var tab = e.target.closest('[data-util-tab]');
      if (tab) {
        var util = tab.getAttribute('data-util-tab');
        if (util && util !== _state.selectedUtil) { _state.selectedUtil = util; _state.expandedGroups = {}; _renderAll(); }
        return;
      }
      var group = e.target.closest('[data-group-id]');
      if (group) {
        var gid = group.getAttribute('data-group-id');
        if (gid) { _state.expandedGroups[gid] = !_state.expandedGroups[gid]; _renderBody(); }
        return;
      }
  ...
  NS.TenantAllocation = {
    ...
    initLive: function (contentEl, ctx) {
      ...
      var datesExpr = (ctx.datesStart && ctx.datesEnd) ? ctx.datesStart + '..' + ctx.datesEnd : 'pastMonth';
      var dateLabel = datesExpr.replace('..', ' – ');

      _state = {
        page: 'summary', selectedUtil: 'Cooling', selectedTenant: null,
        expandedGroups: {}, sortCol: 'pct', sortAsc: false,
        expanded: { billing: true, reconciliation: false, 'res-g1': false, 'res-g2': false, 'res-g3': false, 'res-g4': false, 'res-g5': false, 'res-g6': false },
        allData: { Cooling: [], Heating: [], Flow: [] },
      ...
      var gen = ++_fetchGen;
      Promise.all([
        EVALS.loadAllMeterAllocationUtilities(ctx.attestKey, ctx.projectName, siteRef, datesExpr),
        EVALS.loadAllMeterAllocationSummary(ctx.attestKey, ctx.projectName, siteRef, datesExpr),
        EVALS.loadAllMeterAllocationTenantTotals(ctx.attestKey, ctx.projectName, siteRef, datesExpr),
        EVALS.loadAllMeterAllocationResidential(ctx.attestKey, ctx.projectName, siteRef, datesExpr)
        ...
        allData._residentialData = results[3];

        ['Cooling', 'Heating'].forEach(function (u) {
          var resDataU = allData._residentialData && allData._residentialData[u];
          if (!resDataU || !resDataU.resSum) return;
          var resSum = resDataU.resSum;
          var rows = allData._tenantTotals[u] || [];
          var replaced = false;
          rows.forEach(function (r) {
            if (r.tenantName && r.tenantName.toLowerCase().indexOf('residential') !== -1) {
              r.usage = resSum.val; r.usageUnit = resSum.unit; replaced = true;
            }
          });
          if (!replaced) {
...
})(window.mbcxDashboard.components);
